*Categorical Independent Variables SAS Code;

/*******************************************************
EXAMPLE DATASET: 

Variables:  case = id
			pubs = # of publications 
			time = years since phd
			cits = # of citations
			salary = salary
			female = gender -> female=1 male=0
			age = age of the professor
			rank = 0 = assistant professor
                   1 = associate professor
                   2 = full professor
********************************************************/

data phd;
input case time pubs cits salary female age rank;
cards;
1	3	18	50	51876	1	34	1
2	6	3	26	54511	1	38	0
3	3	2	50	53425	1	31	0
4	8	17	34	61863	0	38	1
5	9	11	41	52926	1	39	1
6	6	6	37	47034	0	34	0
7	16	38	48	66432	0	52	2
8	10	48	56	61100	0	45	2
9	2	9	19	41934	0	40	0
10	5	22	29	47454	0	37	0
11	5	30	28	49832	1	44	1
12	6	21	31	47047	0	49	1
13	7	10	25	39115	1	35	1
14	11	27	40	59677	0	44	1
15	18	37	61	61458	0	59	2
16	6	8	32	54528	0	35	1
17	9	13	36	60327	1	40	1
18	7	6	69	56600	0	41	1
19	7	12	47	52542	1	44	1
20	3	29	29	50455	1	54	0
21	7	29	35	51647	1	38	1
22	5	7	35	42895	0	51	1
23	7	6	18	53740	0	34	0
24	13	69	90	75822	0	61	2
25	5	11	60	56596	0	33	1
26	8	9	30	55682	1	38	2
27	8	20	27	62091	1	41	1
28	7	41	35	42162	1	57	0
29	2	3	14	52646	1	39	0
30	13	27	56	74199	0	50	2
31	5	14	50	50729	0	59	0
32	3	23	25	70011	0	41	1
33	1	1	35	37939	0	41	0
34	3	7	1	39652	0	30	0
35	9	19	69	68987	0	54	1
36	3	11	69	55579	0	49	0
37	9	31	27	54671	0	41	2
38	3	9	50	57704	0	55	0
39	4	12	32	44045	1	48	0
40	10	32	33	51122	0	49	2
41	1	26	45	47082	0	57	1
42	11	12	54	60009	0	43	2
43	5	9	47	58632	0	68	0
44	1	6	29	38340	0	39	0
45	21	39	69	71219	0	59	2
46	7	16	47	53712	1	56	1
47	5	12	43	54782	1	51	0
48	16	50	55	83503	0	49	2
49	5	18	33	47212	0	36	0
50	4	16	28	52840	1	45	0
51	5	5	42	53650	0	52	0
52	11	20	24	50931	0	37	2
53	16	50	31	66784	1	42	2
54	3	6	27	49751	1	39	1
55	4	19	83	74343	1	60	1
56	4	11	49	57710	1	41	1
57	5	13	14	52676	0	39	1
58	6	3	36	41195	1	49	0
59	4	8	34	45662	1	44	0
60	8	11	70	47606	1	59	1
61	3	25	27	44301	1	41	2
62	4	4	28	58582	1	38	0
;
run;

/* Discrete IV with 2 categories (female) */

proc means data=phd;
class female; /* class allows you to compare groups without pre-sorting */
var salary;
run;

proc reg data=phd;
model salary = pubs female;
run; quit;

/* Discrete IV with more than two categories (rank) 

Dummy Coding Table:
--------------------
Category     D1  D2  
Full         0   1   
Associate    1   0   
Assistant    0   0   
--------------------
*/

data dummy;
set phd;
if rank = 1 then D1 = 1;
else D1=0;
if rank = 2 then D2 = 1;
else D2=0;
run;

proc print data=dummy;
run;

proc means data=dummy;
class rank; /* class allows you to compare groups without pre-sorting */
var salary;
run;

proc reg data=dummy;
model salary = pubs D1 D2;
run; quit;

/* Interactions */

proc standard data=dummy out=centered m=0;
var pubs; /* centering pubs for interpretation */
run;

proc reg data=centered;
model salary = pubs D1 D2;
run; quit;

/* Computing the interaction */

data centered;
set centered;
D3 = pubs*D1;
D4 = pubs*D2;
run;

proc print data = centered;
run;

/* Step 3:  Add the interactions (D3 and D4) */

proc reg data=centered;
model salary=pubs D1 D2 D3 D4;
run; quit;

proc means data=centered;
class rank;
var pubs;   /* note the range of pubs for each group for plotting */
run;

/* Get estimates from the model:

salary_hat = B0 + B1*pubs + B2*D1 + B3*D2 + B4*D3 + B5*D4 */

data probe;
set centered;
B0=48589; /*intercept */
B1=-76.43475; /* slope for pubs */
B2=7048.52341; /* slope for D1 */
B3=7174.66251; /* slope for D2 */
B4=341.47676; /* slope of pubs*D1 */
B5=491.66420; /* slope of pubs*D2 */
Full=(B0 + B2) + (B1 + B5)*pubs;
Assoc=(B0 + B3) + (B1 + B4)*pubs;
Assist=B0 + B1*pubs;
run;

proc print data=probe;
run;

/* Plot the simple equations */

goptions reset=all;
symbol1 value=dot color=steel i=join;
symbol2 value=dot color=orange i=join;
symbol3 value=dot color=red i=join;	
legend label=('Professor Rank') value=('Assistant' 'Associate' 'Full') frame;
axis1 label=(angle=-90 rotate=90 'Salary');
axis2 label=('Publications');
title 'Plotting Equations for Groups';
proc gplot data=probe;
plot Assist*pubs Assoc*pubs Full*pubs/overlay vaxis=axis1 haxis=axis2 legend=legend;
run;
quit;
