...
%let macroPath = &root./BIOS-511-FALL-2018-master/programs/2018-11-05-lecture-18/macros;        ** use TEXT SUBSTITUTION to define the MACROPATH macro variable;
%let outPath   = &root./BIOS-511-FALL-2018-master/programs/2018-11-05-lecture-18/output;        ** use TEXT SUBSTITUTION to define the OUTPATH macro variable;
  ...
  %do i = 1 %to &numVars.;
   ...
   %* How do summarize the variables in the dataset one at a time using
      the macro variables I just created;
		%if %upcase(&&type&i) = CHAR %then %do; /* These comparisons in macros ARE case-sensitive */
			title "Frequency Analysis of Variable = &&var&i (&&label&i)"; /* To use text substitution with this macro, you
